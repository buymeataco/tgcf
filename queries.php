...
case 'searchPageResults':
	...
	foreach ($idArray as $value) {
		$nestedMetaValues = [];
		$nestedKeys = [];		
		$query2 = "SELECT meta_value FROM metaTest WHERE meta_key IN ('first_name','last_name','city','employer','title') AND user_id = '$value'";
		$result2 = mysqli_query($conn,$query2) or die ("<br />Could not execute second query.");	
		$resultCountMetaValues = mysqli_num_rows($result2);
